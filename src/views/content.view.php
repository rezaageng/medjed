...
<main class="max-w-4xl m-auto p-4 mb-16">
  <div class="flex flex-col gap-4 ">
    <h1 class="text-3xl text-center"><?= htmlspecialchars($item['title']) ?></h1>
    <a class="flex justify-center " href="print.php?id=<?= htmlspecialchars($item['id']) ?>" target="_blank">
    ...
    <img src="./images/<?= htmlspecialchars($item['image']) ?>" alt="image" class="w-full">
    <div class="text-justify rte-content"><?= $item['content'] ?></div>
  </div>
  <div class="mt-4 flex flex-col gap-2 border border-t-white border-b-transparent border-l-transparent border-r-transparent pt-4">
    ...
    <form action="" method="post">
      <input name="newsId" type="hidden" value="<?= htmlspecialchars($item['id']) ?>">
    ...
    <?php foreach ($comments as $comment) : ?>
      <div class="bg-gray-700 flex flex-col gap-2 p-4 rounded-lg">
        <p class="text-justify"><?= htmlspecialchars($comment['comment']) ?></p>
        <span class="text-xs"><?= htmlspecialchars($comment['created_at']) ?></span>
      </div>
    <?php endforeach; ?>
  </div>
</main>
